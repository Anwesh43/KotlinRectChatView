...
class RectChatView(ctx : Context) : View(ctx) {
    ...
    data class RectChatNode(var i : Int , val state : State = State()) {
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            ...
            clipPath.addRect(RectF(xStart, -h_Gap/2, xStart + (w - 3 * h_Gap) * state.scales[1], h_Gap/2), Path.Direction.CCW)
            canvas.clipPath(clipPath)
            canvas.save()
            canvas.translate(xStart, 0f)
            val path : Path = Path()
            path.moveTo(0f, 0f)
            path.lineTo(h_Gap/4 + h_Gap/20, -h_Gap/4)
            path.lineTo(h_Gap/4 + h_Gap/20, h_Gap/4)
            canvas.drawPath(path, paint)
            canvas.drawRect(RectF(h_Gap/4, -h_Gap/2, w - 2.75f * h_Gap, h_Gap/2), paint)
            canvas.restore()
            canvas.restore()
            canvas.restore()
            canvas.restore()
